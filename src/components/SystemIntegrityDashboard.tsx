...
export const SystemIntegrityDashboard: React.FC<SystemIntegrityDashboardProps> = ({
  ...
  const hsmTheme = {
    nominal_cold: {
      ...
      tag: 'MODELLED (SR-2 NOT PROVABLE)',
      description: 'The model holds no profile key in this state, and the Weaver rate-limit barrier is armed. SR-2 itself is not shown here: a browser cannot zero its own memory, so "no keys in RAM" is a target for a native port, not a property of this page (finding F-2).',
